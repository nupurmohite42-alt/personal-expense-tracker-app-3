...
def save_budget(df: pd.DataFrame):
...
def show_exit_screen():
    ...
    # Allow user to restart app without killing the whole session
...
# ---------------- SET BUDGET ----------------
    ...
    if st.button("Save Budget"):
        try:
            # Normalize user input to YYYY-MM using Period
            ...
            budget_df = pd.concat(
                [
                    budget_df,
                    pd.DataFrame([{"Month": month, "Budget": budget_amount}]),
                ],
                ignore_index=True,
            )
...
# ---------------- DELETE EXPENSE ----------------
    ...
    if expense_df.empty:
        ...
        # Add an index column to show a user-friendly id
        expense_df = expense_df.reset_index().rename(columns={"index": "RowID"})

        st.write("Select a row to delete:")
        st.dataframe(expense_df, use_container_width=True)

        # Let user pick by RowID
        row_ids = expense_df["RowID"].tolist()
        selected_row_id = st.selectbox("RowID to delete", row_ids)

        if st.button("Delete Selected Expense"):
            # Map RowID back to original index in main df
            original_index = expense_df.loc[
                expense_df["RowID"] == selected_row_id, "RowID"
            ].values[0]

            df_exp_only = df[df["Category"] != "Income"].reset_index()
            original_global_index = df_exp_only.loc[
                df_exp_only["index"] == original_index, "index"
            ].values[0]

            df = df.drop(index=original_global_index).reset_index(drop=True)
            save_expenses(df)
